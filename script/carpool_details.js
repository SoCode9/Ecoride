      ...
      .then((data) => {
        if (data.success) {
          ...
          if (data.userCredits < data.travelPrice) {
            alert(
              "Vous n'avez pas assez de crédits pour réserver ce covoiturage"
            );
            return;
          }

          // double confirmation
          let confirmParticipation = confirm(
            "Souhaitez-vous vraiment participer à ce covoiturage ?"
          );
        ...
        if (data.message && data.message.includes("Utilisateur non connecte")) {
          let confirmLogin = confirm(
            'Vous devez être connecté pour réserver. Cliquer sur "OK" pour créer un compte.'
          );
          if (confirmLogin) {
        ...
        if (
          ...
          data.message.includes(
            "Le covoiturage est soit en cours, soit annulé, soit terminé"
          )
        ) {
